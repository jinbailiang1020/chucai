...
import com.sm.sls_app.ui.Bet_YNSSCActivity.MyAsynTask;
import com.sm.sls_app.ui.Bet_YNSSCActivity.MyCheckChange;
import com.sm.sls_app.ui.Bet_YNSSCActivity.MyHandler;
import com.sm.sls_app.ui.Bet_YNSSCActivity.listItemClick;
import com.sm.sls_app.ui.adapter.MyBetLotteryYNSSCAdapter;
 ...
 * @author huang 印尼时时彩
 */
public class Bet_YNSSCActivity extends Activity implements OnClickListener,
    Refresh {
  ...
  public MyBetLotteryYNSSCAdapter adapter;
  ...
  private void findView() {
    ...
    bet_btn_deleteall = (ImageView) this.findViewById(R.id.bet_btn_deleteall);
    ...
    bet_cb_stopfollow = (CheckBox) this.findViewById(R.id.bet_cb_stopfollow);
    ...
    btn_playtype.setText("印尼时时彩投注单");

    adapter = new MyBetLotteryYNSSCAdapter(Bet_YNSSCActivity.this,
        AppTools.list_numbers);
    zAdapter = new ZhuiqiAdapter(Bet_YNSSCActivity.this,
  ...
  private void init() {
    ...
    if (null != bundle) {
      ...
      if (type == 9) {
        ...
        // 胆拖号码
      } else if (playType == 9316 || playType == 9317) {
      ...
      numbers.setNumber((one + "-" + two + "-" + three + "-" + four + "-"
          + five + dan.replace(",", "") + "-" + tuo.replace(",", "")).replace(
          "(", "").replace(")", ""));
  ...
  class listItemClick implements OnItemClickListener {
    ...
    public void onItemClick(AdapterView<?> arg0, View arg1, int position,
      ...
      Bet_YNSSCActivity.this.startActivity(intent);
    }
  }
      ...
      if (edt.toString().trim().length() != 0) {
        if (Integer.parseInt(edt.toString().trim()) > AppTools.lottery
          ...
          MyToast.getToast(getApplicationContext(),
              "最多可追" + AppTools.lottery.getDtCanChaseIsuses().size() + "期")
              .show();
        }
        ...
        if (edt.toString().substring(0, 1).equals("0")) {
          et_qi.setText(edt.toString().subSequence(1, edt.toString().length()));
  ...
  private TextWatcher bei_textWatcher = new TextWatcher() {
    ...
    public void afterTextChanged(Editable edt) {
      ...
      if (edt.toString().trim().length() != 0) {
        ...
        if (edt.toString().substring(0, 1).equals("0")) {
          et_bei
              .setText(edt.toString().subSequence(1, edt.toString().length()));
  ...
  public void onClick(View v) {
    ...
    case R.id.bet_btn_deleteall:
      if (null != AppTools.list_numbers && 0 != AppTools.list_numbers.size()) {
        dialog.show();
    ...
    case R.id.bet_tv_guize:
      Intent intent = new Intent(Bet_YNSSCActivity.this, PlayDescription.class);
  ...
  private void join() {
    ...
    intent = new Intent(Bet_YNSSCActivity.this, JoinActivity.class);
  ...
  private void btn_randomClick(int count) {
    ...
    if (playType == 9303) {
      ...
      case 2:
        ...
        ShowLotterynumber = number.replace("[", "").replace("]", "")
            .replace(",", "").replace(" ", "");
        number1 = number.replace("[", "").replace("]", "").replace(" ", "")
            .split(",")[0]
            + "-"
            + number.replace("[", "").replace("]", "").replace(" ", "")
                .split(",")[1];
    ...
    protected String doInBackground(Void... params) {
      ...
      if (AppTools.list_numbers.size() > 0) {
        ...
        if (AppTools.user != null) {
          ...
          // }
          // else{
          info = RspBodyBaseBean.changeBet_info_ssc("93", AppTools.lottery
              .getIsuseId(), AppTools.bei, 1, 1, 0, 0, "", "", 0, totalMoney
              / AppTools.qi, sumCount, AppTools.qi > 1 ? AppTools.qi : 0,
  ...
  class MyHandler extends Handler {
    ...
    public void handleMessage(Message msg) {
      ...
      case -160:
        ...
        Toast.makeText(getApplicationContext(), msgs, Toast.LENGTH_LONG).show();
        AppTools.list_numbers.clear();
        AppTools.beiList.clear();
        AppTools.totalCount = 0;
        break;
      ...
      case AppTools.ERROR_MONEY:
        Toast.makeText(Bet_YNSSCActivity.this, "余额不足", Toast.LENGTH_SHORT).show();
        intent = new Intent(Bet_YNSSCActivity.this, RechargeActivity.class);
        intent.putExtra("money", totalMoney);
        Bet_YNSSCActivity.this.startActivity(intent);
        break;
      /** 尚未登陆 **/
      case AppTools.ERROR_UNLONGIN:
        MyToast.getToast(Bet_YNSSCActivity.this, "请先登陆").show();
        intent = new Intent(Bet_YNSSCActivity.this, LoginActivity.class);
        intent.putExtra("loginType", "bet");
        Bet_YNSSCActivity.this.startActivity(intent);
        break;
      /** 点击付款时 所选注数为 0 **/
      case AppTools.ERROR_TOTAL:
        MyToast.getToast(Bet_YNSSCActivity.this, "请至少选择一注").show();
        intent = new Intent(Bet_YNSSCActivity.this, Select_YNSSCActivity.class);
        intent.putExtra("loginType", "bet");
        Bet_YNSSCActivity.this.startActivity(intent);
        break;
      case -500:
        MyToast.getToast(Bet_YNSSCActivity.this, "连接超时").show();
        break;
      default:
        Toast.makeText(Bet_YNSSCActivity.this, "网络异常，购买失败。请重新点击付款购买。",
