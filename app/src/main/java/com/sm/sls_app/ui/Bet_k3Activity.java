...
import com.sm.sls_app.ui.adapter.GridView_k3Adapter;
import com.sm.sls_app.ui.adapter.MyBetLotteryListAdapter;
import com.sm.sls_app.ui.adapter.MyBetLotteryList_k3Adapter;
 ...
 * 
 */
public class Bet_k3Activity extends Activity implements OnClickListener,
		Refresh {
	...
	private void findView() {
		...
		btn_playtype.setText("江苏快3投注单");

		adapter = new MyBetLotteryList_k3Adapter(Bet_k3Activity.this,
				AppTools.list_numbers);
		zAdapter = new ZhuiqiAdapter(Bet_k3Activity.this,
	...
	private void init() {
		...
		if (null != bundle) {
			...
			Log.i(TAG, "playType" + playType);
			Log.i(TAG, "one" + one + "___");
			Log.i(TAG, "two" + bundle.getString("two") + "___");
			String two = "";
			...
			} else if (playType == 8305 || 8309 == playType || 8310 == playType) {
				String[] array = one.replace(" ", "").split(",");
	...
	class listItemClick implements OnItemClickListener {
		...
		public void onItemClick(AdapterView<?> arg0, View arg1, int position,
			...
			// 跳转页面并传值
			intent = new Intent(Bet_k3Activity.this, Select_k3Activity.class);
			...
			if (clickType == 8301 || clickType == 8307 || clickType == 8306) {
				...
				listOne.add("1");
				bundle.putStringArrayList("oneSet", listOne);
			} else if (clickType == 8305 || clickType == 8309
					|| clickType == 8310) {
			...
			Bet_k3Activity.this.startActivity(intent);
	...
	public void onClick(View v) {
		...
		case R.id.bet_tv_guize:
			Intent intent = new Intent(Bet_k3Activity.this,
	...
	private void join() {
		...
		intent = new Intent(Bet_k3Activity.this, JoinActivity.class);
	...
	private void btn_randomClick(int count) {
		...
		switch (playType) {
		case 8301:
			list = NumberTools.getRandomNumk_3(1, 3, 18); // 随机1个数 大于等3 小于19
			...
			if (AppTools.list_numbers.size() > 0) {
				...
				if (AppTools.user != null) {
					String key = MD5.md5(AppTools.user.getUserPass()
							+ AppTools.MD5_key);
					info = RspBodyBaseBean.changeBet_info2("83",
	...
	class MyHandler extends Handler {
		...
		public void handleMessage(Message msg) {
			...
			case AppTools.ERROR_MONEY:
				Toast.makeText(Bet_k3Activity.this, "余额不足", Toast.LENGTH_SHORT)
						.show();
				intent = new Intent(Bet_k3Activity.this, RechargeActivity.class);
				intent.putExtra("money", totalMoney);
				Bet_k3Activity.this.startActivity(intent);
				break;
			/** 尚未登陆 **/
			case AppTools.ERROR_UNLONGIN:
				MyToast.getToast(Bet_k3Activity.this, "请先登陆").show();
				intent = new Intent(Bet_k3Activity.this, LoginActivity.class);
				intent.putExtra("loginType", "bet");
				Bet_k3Activity.this.startActivity(intent);
				break;
			/** 点击付款时 所选注数为 0 **/
			case AppTools.ERROR_TOTAL:
				MyToast.getToast(Bet_k3Activity.this, "请至少选择一注").show();
				intent = new Intent(Bet_k3Activity.this,
						Select_k3Activity.class);
				intent.putExtra("loginType", "bet");
				Bet_k3Activity.this.startActivity(intent);
				break;
			case -500:
				MyToast.getToast(Bet_k3Activity.this, "连接超时").show();
				break;
			default:
				Toast.makeText(Bet_k3Activity.this, "网络异常，购买失败。请重新点击付款购买。",
