 ...
 * 
 */
public class WinLottery_ssq_Activity extends Activity implements
		OnClickListener {
	...
	public void onClick(View view) {
		...
		case R.id.btn_to_bet:
			...
			for (Lottery lottery : HallFragment.listLottery) {
				if (lottery.getLotteryID().equals(winLottery.getLotteryId())) {
					if (lottery.getDistanceTime() - currentTime <= 0) {
						MyToast.getToast(WinLottery_ssq_Activity.this,
								"该奖期已结束，请等下一期").show();
						return;
					}
				}
			...
			for (Lottery lottery : HallFragment.listLottery) {
				if (lottery.getLotteryID().equals("5")&&winLottery.getLotteryId().equals("5")) {
					AppTools.lottery = lottery;
					intent = new Intent(WinLottery_ssq_Activity.this,
							SelectNumberActivity.class);
				}
			}
			WinLottery_ssq_Activity.this.startActivity(intent);
			break;
