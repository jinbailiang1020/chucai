...
import com.sm.sls_app.ui.adapter.MyBetLotteryPK10Adapter;
...
/**
 * @author 作者 : hxj
 * @version 创建时间：2016-4-9 上午9:57:04 类说明
 */
public class Bet_BJPK10Activity extends Activity implements OnClickListener,
		Refresh {

	private final static String TAG = "Bet_BJPK10Activity";
	/* 头部 */
	...
	public MyBetLotteryPK10Adapter adapter;
	...
	private void findView() {
		...
		btn_playtype.setText("北京PK10投注单");

		adapter = new MyBetLotteryPK10Adapter(Bet_BJPK10Activity.this,
				AppTools.list_numbers);
		zAdapter = new ZhuiqiAdapter(Bet_BJPK10Activity.this,
	...
	private void init() {
		...
		if (null != bundle) {
			/**
			 * one=01, 02, two=04, 05, five=, four=, three=07, 10, playType=9403
			 */
			...
			if (playType == 9401) {
				...
				numbers.setShowLotteryNumber(one + two.replace(",", ""));
				numbers.setLotteryNumber(one + two.replace(",", ""));
			} else if (playType == 9403) {
				numbers.setShowLotteryNumber(one + two + three.replace(",", ""));
				numbers.setLotteryNumber(one + two + three.replace(",", ""));
	...
	class listItemClick implements OnItemClickListener {
		...
		public void onItemClick(AdapterView<?> arg0, View arg1, int position,
			...
			// 跳转页面并传值
			intent = new Intent(Bet_BJPK10Activity.this,
			...
			Log.i("x", "pk10=str====" + num.getNumber());
			...
			// 将Bundle 放入Intent
			intent.putExtra("PK10Bundle", bundle);

			AppTools.list_numbers.remove(position);

			Bet_BJPK10Activity.this.startActivity(intent);
		}
	}
	...
	public void onClick(View v) {
		...
		case R.id.bet_tv_guize:
			Intent intent = new Intent(Bet_BJPK10Activity.this,
	...
	private void join() {
		...
		intent = new Intent(Bet_BJPK10Activity.this, JoinActivity.class);
		...
		protected String doInBackground(Void... params) {
			...
			if (AppTools.list_numbers.size() > 0) {
				...
				if (AppTools.user != null) {
					...
					// }
					// else{
					// info = RspBodyBaseBean.changeBet_info_ssc("94",
					// AppTools.lottery.getIsuseId(), AppTools.bei, 1, 1,
					// 0, 0, "", "", 0, totalMoney / AppTools.qi,
					// sumCount, AppTools.qi > 1 ? AppTools.qi : 0,
					// AppTools.qi == 1 ? 0 : totalMoney,
					// AppTools.list_numbers, isStopChase);
					info = RspBodyBaseBean.changeBet_info2("94",
							AppTools.lottery.getIsuseId(), AppTools.bei, 1, 1,
							0, 0, "", "", 0, totalMoney / AppTools.qi,
							sumCount, AppTools.qi > 1 ? AppTools.qi : 0,
							AppTools.qi == 1 ? 0 : totalMoney,
					...
					Log.i("x", "betActivity PK10--info" + info);
	...
	class MyHandler extends Handler {
		...
		public void handleMessage(Message msg) {
			...
			case AppTools.ERROR_MONEY:
				Toast.makeText(Bet_BJPK10Activity.this, "余额不足",
			...
			case AppTools.ERROR_UNLONGIN:
				MyToast.getToast(Bet_BJPK10Activity.this, "请先登陆").show();
				intent = new Intent(Bet_BJPK10Activity.this,
						LoginActivity.class);
			...
			case AppTools.ERROR_TOTAL:
				MyToast.getToast(Bet_BJPK10Activity.this, "请至少选择一注").show();
				intent = new Intent(Bet_BJPK10Activity.this,
						Select_BJPK10Activity.class);
				intent.putExtra("loginType", "bet");
				Bet_BJPK10Activity.this.startActivity(intent);
				break;
			case -500:
				MyToast.getToast(Bet_BJPK10Activity.this, "连接超时").show();
				break;
			default:
				Toast.makeText(Bet_BJPK10Activity.this, "网络异常，购买失败。请重新点击付款购买。",
						Toast.LENGTH_SHORT).show();
				break;
			}
