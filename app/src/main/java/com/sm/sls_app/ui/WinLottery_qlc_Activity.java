 ...
 * 
 */
public class WinLottery_qlc_Activity extends Activity implements
		OnClickListener {
	...
	private TextView tv_one_count, tv_two_count, tv_three_count, tv_four_count,
			tv_five_count, tv_six_count, tv_seven_count;
	// 每注奖金
	...
	protected void onCreate(Bundle savedInstanceState) 
	{
		...
		setContentView(R.layout.activity_winlottery_number_qlc);
		...
		case R.id.btn_to_bet:
			...
			for (Lottery lottery : HallFragment.listLottery) {
				if(lottery.getLotteryID().equals(winLottery.getLotteryId())){
					if(lottery.getDistanceTime()-currentTime <= 0){
						MyToast.getToast(WinLottery_qlc_Activity.this, "该奖期已结束，请等下一期").show();
						return; 
					}
 				}
			}
			...
			for (Lottery lottery : HallFragment.listLottery) 
			{
				if (lottery.getLotteryID().equals("13")) 
				{
					...
					Intent intent = new Intent(WinLottery_qlc_Activity.this,
							Select_QlcActivity.class);
					WinLottery_qlc_Activity.this.startActivity(intent);
				}
			}
			break;
