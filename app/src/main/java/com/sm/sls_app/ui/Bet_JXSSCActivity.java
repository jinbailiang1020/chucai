...
import com.sm.sls_app.ui.adapter.MyBetLotteryJXSSCAdapter;
...
/** 功能：投注页面 版本 */
public class Bet_JXSSCActivity extends Activity implements OnClickListener {
	...
	public MyBetLotteryJXSSCAdapter adapter;
	...
	private void findView() {
		...
		btn_playtype.setText("江西时时彩投注单");

		adapter = new MyBetLotteryJXSSCAdapter(Bet_JXSSCActivity.this,
	...
	private void init() {
		...
		if (null != bundle) {
			...
			if (type == 9) {
				...
				// 胆拖号码
			} else if (playType == 6114 || playType == 6119) {
	...
	class listItemClick implements OnItemClickListener {
		...
		public void onItemClick(AdapterView<?> arg0, View arg1, int position,
			...
			// 跳转页面并传值
			intent = new Intent(Bet_JXSSCActivity.this,
			...
			// 胆拖的话
			if (num.getPlayType() == 6114 || num.getPlayType() == 6119) {
				if (num.getPlayType() == 6114) {
					bundle.putString("Bet_sxzsdt", num.getLotteryNumber());
					Log.i("三星组三胆拖", "--" + num.getShowLotteryNumber() + "---");
				} else if (num.getPlayType() == 6119) {
			...
			Bet_JXSSCActivity.this.startActivity(intent);
		}
	}
	...
	public void onClick(View v) {
		...
		case R.id.bet_tv_guize:
			Intent intent = new Intent(Bet_JXSSCActivity.this,
	...
	private void join() {
		...
		intent = new Intent(Bet_JXSSCActivity.this, JoinActivity.class);
	...
	private void btn_handClick() {
		...
		intent = new Intent(Bet_JXSSCActivity.this, Select_JXSSCActivity.class);
		Bundle bundle = new Bundle();
		...
		protected String doInBackground(Void... params) {
			...
			if (AppTools.list_numbers.size() > 0) {
				...
				if (AppTools.user != null) {
					...
					// }
					// else{
					info = RspBodyBaseBean.changeBet_info_ssc("61",
					...
					Log.i("x", "Bet_k3Activity SSC--info" + info);
	...
	class MyHandler extends Handler {
		...
		public void handleMessage(Message msg) {
			...
			case AppTools.ERROR_MONEY:
				Toast.makeText(Bet_JXSSCActivity.this, "余额不足",
			...
			case AppTools.ERROR_UNLONGIN:
				MyToast.getToast(Bet_JXSSCActivity.this, "请先登陆").show();
				intent = new Intent(Bet_JXSSCActivity.this, LoginActivity.class);
				intent.putExtra("loginType", "bet");
				Bet_JXSSCActivity.this.startActivity(intent);
				break;
			/** 点击付款时 所选注数为 0 **/
			case AppTools.ERROR_TOTAL:
				MyToast.getToast(Bet_JXSSCActivity.this, "请至少选择一注").show();
				intent = new Intent(Bet_JXSSCActivity.this,
						Select_JXSSCActivity.class);
				intent.putExtra("loginType", "bet");
				Bet_JXSSCActivity.this.startActivity(intent);
				break;
			case -500:
				MyToast.getToast(Bet_JXSSCActivity.this, "连接超时").show();
				break;
			default:
				Toast.makeText(Bet_JXSSCActivity.this, "网络异常，购买失败。请重新点击付款购买。",
						Toast.LENGTH_SHORT).show();
				break;
			}
